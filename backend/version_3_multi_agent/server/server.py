...
from server.task_manager import TaskManager              # Our actual task handling logic (Gemini agent)
...
# -----------------------------------------------------------------------------
class A2AServer:
    def __init__(self, host="0.0.0.0", port=5000, agent_card: Optional[AgentCard] = None, task_manager: Optional[TaskManager] = None):
        """
    ...
    def _get_agent_card(self, request: Request) -> JSONResponse:
        ...
        if not self.agent_card:
            return JSONResponse({"error": "Agent card not configured"}, status_code=404)
        ...
        try:
            ...
            if isinstance(json_rpc, SendTaskRequest):
                if not self.task_manager:
                    raise ValueError("Task manager not configured")

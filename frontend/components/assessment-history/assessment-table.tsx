...
import { ArrowRight, AlertTriangle, CheckCircle, Activity, User, CalendarDays } from "lucide-react";
              ...
              <TableRow key={assessment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors duration-150">
                <TableCell className="px-4 py-3 whitespace-nowrap text-sm font-medium text-slate-800 dark:text-slate-100">
                  <div className="flex items-center">
                    ...
                    <span className="truncate" title={assessment.patientName || `ID: ${assessment.id}`}>
                      {assessment.patientName || `ID: ${assessment.id}`}
                    </span>
                  </div>
                </TableCell>
                ...
                <TableCell className="px-4 py-3 hidden md:table-cell text-xs text-slate-500 dark:text-slate-400">
                  <span className="line-clamp-2" title={assessment.symptomDescription}>
                    {assessment.symptomDescription || "N/A"}
                  </span>
                </TableCell>
                <TableCell className="px-4 py-3 text-center">
                  <Badge variant="outline" className={`px-2 py-0.5 text-xs capitalize flex items-center justify-center gap-1 ${statusColors[status]}`}>
                    {statusIcons[status]}
                    {status}
                  </Badge>
                </TableCell>
                <TableCell className="px-4 py-3 text-center hidden lg:table-cell text-sm font-semibold text-slate-700 dark:text-slate-200">
                  {assessment.severityScore ? `${assessment.severityScore}/5` : 'N/A'}
                </TableCell>
                <TableCell className="px-4 py-3 hidden sm:table-cell text-xs text-slate-500 dark:text-slate-400">
                  {formatDate(assessment.createdAt)}
                </TableCell>
                <TableCell className="px-4 py-3 hidden lg:table-cell text-xs text-slate-500 dark:text-slate-400">
